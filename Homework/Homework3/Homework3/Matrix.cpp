...
void MatrixOps::populate(map<int, int> dict, string filename, int** matrix)
{
...
list<Node*> MatrixOps::matrixToList(map<int, int> dict, int** matrix, list<Node*> V)
{
	Node *temp = NULL;
	for (int i = 0; i < matrixSize; i++)
	{
...
the next row.*/
int ** MatrixOps::listToMatrix(map<int, int>dict, list<Node*> V, int** matrix)
{
	initMatrix(matrix);
	list<int>tempC;
	Node *temp = NULL;
...
/*vector<std::pair<int, int>> MatrixOps::checkSymm(int **matrix)
{
